<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form class="form-reset" action="<?= base_url('admin/usuarios/guardar') ?>" method="POST"
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-control" id="rol" name="rol" required>
                            <option value="" disabled <?= !isset($usuario) ? 'selected' : '' ?>>Seleccione un rol...
                            </option>
                            <option value="admin"
                                <?= (isset($usuario) && $usuario['rol'] == 'admin') ? 'selected' : '' ?>>Administrador
                            </option>
                            <option value="estudiante"
                                <?= (isset($usuario) && $usuario['rol'] == 'estudiante') ? 'selected' : '' ?>>Estudiante
                            </option>
                            <option value="propietario"
                                <?= (isset($usuario) && $usuario['rol'] == 'propietario') ? 'selected' : '' ?>>
                                Propietario</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label for="password" class="form-label font-weight-bold">
                            Contraseña
                            <?= isset($usuario) ? '<small class="text-muted">(Dejar en blanco para no cambiar)</small>' : '' ?>
                        </label>
                    ...
                    <div class="col-md-6 mb-3">
                        <label for="foto_perfil" class="form-label font-weight-bold">Foto de Perfil</label>
                        <input type="file" class="form-control" id="foto_perfil" name="foto_perfil" accept="image/*">

                        <?php if (!empty($usuario['foto_perfil'])): ?>
                        <div class="mt-2">
                            <small class="text-muted d-block mb-1">Imagen actual:</small>
                            <img src="<?= base_url('uploads/perfiles/' . $usuario['foto_perfil']) ?>" alt="Perfil"
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-control" id="universidad_id" name="universidad_id">
                            <option value="" <?= !isset($usuario['universidad_id']) ? 'selected' : '' ?>>Ninguna / No
                                aplica</option>
                            <?php foreach($universidades as $uni): ?>
                            <option value="<?= $uni['id'] ?>"
                                <?= (isset($usuario) && $usuario['universidad_id'] == $uni['id']) ? 'selected' : '' ?>>
                                <?= $uni['nombre'] ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-control" id="id_carreara" name="id_carreara">
                            <option value="" <?= !isset($usuario['id_carrera']) ? 'selected' : '' ?>>Ninguna / No aplica
                            </option>
                            <?php foreach($carreras as $car): ?>
                            <option value="<?= $car['id'] ?>"
                                <?= (isset($usuario) && $usuario['id_carrera'] == $car['id']) ? 'selected' : '' ?>>
                                <?= $car['nombre'] ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        <label for="link_instagram" class="form-label font-weight-bold"><i class="fab fa-instagram"></i>
                            Instagram</label>
                        <input type="text" class="form-control" id="link_instagram" name="link_instagram"
                            value="<?= $usuario['link_instagram'] ?? '' ?>" placeholder="@usuario">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label for="link_x" class="form-label font-weight-bold"><i class="fab fa-twitter"></i> X
                            (Twitter)</label>
                        <input type="text" class="form-control" id="link_x" name="link_x"
                            value="<?= $usuario['link_x'] ?? '' ?>" placeholder="@usuario">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label for="link_spotify" class="form-label font-weight-bold"><i class="fab fa-spotify"></i>
                            Spotify</label>
